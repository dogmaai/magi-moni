...
async function getOpenClawUrl() {
  if (process.env.OPENCLAW_URL) return process.env.OPENCLAW_URL;
...
/**
 * Run a shell command on TIALA.
 * Prefers the direct `exec` tool on /tools/invoke. Some OpenClaw builds
 * (e.g. 2026.6.1) do not expose `exec` in the HTTP tool catalog; in that case
 * this falls back to a one-turn agent run on /v1/chat/completions, which has
 * the full tool catalog, instructing the agent to run the exact command and
 * return stdout verbatim.
 * @param {string} command - Shell command to run.
 * @param {number} [timeoutSec=30]
 * @returns {Promise<{stdout: string, via: string, raw: object}>}
 */
async function callOpenClawExec(command, timeoutSec = 30) {
  try {
    const res = await callOpenClaw('exec', {
      command,
      host: 'gateway',
      security: 'full',
      timeout: timeoutSec,
    ...
    return { stdout: typeof stdout === 'string' ? stdout : JSON.stringify(stdout), via: 'tools/invoke', raw: res };
  } catch (e) {
    if (!/Tool not available/i.test(e.message)) throw e;
  }
  const instruction = [
    'You are a non-interactive command runner.',
    'Use the exec tool to run exactly this shell command, without modifying it:',
    '```',
    command,
    '```',
    'Reply with the raw stdout of the command only — no commentary, no markdown fences.'
  ].join('\n');
  const res = await callOpenClawAgent(instruction, { timeout: (timeoutSec + 90) * 1000 });
  // Read the assistant message directly: res.content substitutes the whole
  // response envelope when the message is empty, which must not be mistaken
  // for command output.
  const message = res.raw?.choices?.[0]?.message?.content;
  return { stdout: typeof message === 'string' ? message : '', via: 'chat/completions', raw: res };
}

module.exports = { callOpenClaw, callOpenClawAgent, callOpenClawExec };
