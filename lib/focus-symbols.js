...
async function addFocusSymbol(symbol, { addedBy = 'AKA-1 Telegram', note = null } = {}) {
  ...
  const rows = await runQuery(
    `SELECT
       COUNTIF(active) AS active_count,
       COUNTIF(active AND symbol = @symbol) AS already_active
     FROM ${MANUAL_TABLE}`,
    { symbol: sym }
  );
  const active = Number(rows[0]?.active_count || 0);
  const alreadyActive = Number(rows[0]?.already_active || 0) > 0;
  if (!alreadyActive && active >= MAX_MANUAL_SYMBOLS) {
  ...
  return { symbol: sym, active_count: alreadyActive ? active : active + 1 };
}
