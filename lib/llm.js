...
const SYSTEM_PROMPT_CORE =
  ...
  '\n- openclaw_agent: OpenClaw エージェント（Sonnet 5）に自然言語で指示。TIALAの画面を見ながらタスクを実行' +
  '\ntiala_services, tiala_system, tiala_screenshot は確認不要で即時実行可能。' +
  '\ntiala_restart, tiala_exec, tiala_action は confirmed=true が必要（初回は確認なしでポリシーチェック→ユーザー確認→再実行）。';
