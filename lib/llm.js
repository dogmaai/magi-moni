...
// Prefix telling the user the primary model failed and which model actually answered.
function fallbackNotice(errors, respondingModel) {
  if (errors.length === 0) return null;
  return `⚠️ フォールバック: ${respondingModel} が応答しています（${errors.join(' / ')}）`;
}
...
async function handleAka1Chat(chatId, text) {
  ...
  if (OLLAMA_BASE_URL) {
    try {
      ...
      await sendAnswerWithPhoto(chatId, answer, chatStartMs, fallbackNotice(errors, `Ollama(${OLLAMA_MODEL})`));
      return;
    } catch (e) {
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      await sendAnswerWithPhoto(chatId, answer, chatStartMs, fallbackNotice(errors, `Gemini(${GEMINI_FALLBACK_MODEL})`));
      return;
    } catch (e) {
