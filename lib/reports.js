...
function failureSection(sections) {
  ...
  let out = `\n\u26a0\ufe0f <b>\u53d6\u5f97\u5931\u6557</b>\n`;
  for (const [name, r] of failed) out += `  ${name}: ${r.error}\n`;
  return out;
}
...
async function generateDailyReport() {
  ...
  if (l4Res.error) {
    ...
    for (const b of l4Warnings) {
      const who = b.unit_name || b.llm_provider;
      msg += `  ${who}(${b.side || '-'}): ${b.warnings}件 最終 ${fmtTimestamp(b.last_warned_at)}\n`;
    }
    msg += '\n';
  } else {
    msg += `✅ L4警告(直近7日): なし\n\n`;
...
async function generateWeeklyReport() {
  ...
  if (patterns.length > 0) {
    ...
    for (const p of patterns) {
      const summary = String(p.win_pattern_summary || '').replace(/\s+/g, ' ').replace(/^\**\s*/, '').slice(0, 160);
      msg += `  ${p.llm_provider} (${p.win_count}W/${p.lose_count}L): ${summary}\n`;
    }
  }
