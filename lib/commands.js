...
async function handleBotCommand(chatId, text) {
  const cmd = text.split(' ')[0].toLowerCase().replace(/@magi_[a-z0-9_]*_bot\b/gi, '');
  ...
  if (cmd === '/help' || cmd === '/start') {
    return sendTelegramTo(chatId,
      ...
      `/today   - 本日の取引一覧\n` +
      `/focus   - HERMES監視銘柄一覧 (/focus add PLTR で追加, /focus del PLTR で解除)\n` +
      `/kill    - 緊急キルスイッチ発動（全注文ブロック。/kill 理由 で理由付き, /kill status で状態確認）\n` +
      `/resume  - キルスイッチ解除（取引再開）\n` +
      ...
      `例: 「TIALAのサービス状態を確認して」「Ollamaを再起動して」「ollama listを実行して」「TIALAの画面を撮って（Telegramに画像送信）」「TIALAの画面でクリックして」`
    );
  }
  ...
  if (cmd === '/focus') {
    try {
      ...
      if (sub === 'add' || sub === 'del' || sub === 'remove' || sub === 'rm') {
        const symbol = args[1];
        if (!symbol) {
          return sendTelegramTo(chatId, `[MAGI] 使い方: /focus ${sub} <銘柄>（例: /focus ${sub} PLTR）`);
        }
        if (sub === 'add') {
          const note = args.slice(2).join(' ') || null;
          const result = await addFocusSymbol(symbol, { addedBy: `telegram:${chatId}`, note });
          return sendTelegramTo(chatId,
            `[MAGI] 監視銘柄に追加しました: ${result.symbol}\n` +
            `手動監視: ${result.active_count}/${MAX_MANUAL_SYMBOLS}件\n` +
            `HERMESが次回収集からニュース/センチメントを収集します`);
        }
        const result = await removeFocusSymbol(symbol);
        return sendTelegramTo(chatId, result.removed
          ? `[MAGI] 監視銘柄から解除しました: ${result.symbol}`
          : `[MAGI] ${result.symbol} は手動監視銘柄に登録されていません`);
      }

      const { manual, isabel } = await listFocusSymbols();
      let msg = `[MAGI] HERMES監視銘柄\n\n`;
      msg += `手動追加 (${manual.length}/${MAX_MANUAL_SYMBOLS}件):\n`;
      msg += manual.length > 0
        ? manual.map((r) => `  ${r.symbol}${r.note ? ` - ${r.note}` : ''}`).join('\n') + '\n'
        : '  なし\n';
      msg += `\nISABEL自動選定 (本日 ${isabel.length}件):\n`;
      msg += isabel.length > 0
        ? isabel.map((r) => `  #${r.rank} ${r.symbol} (WR ${r.win_rate != null ? Number(r.win_rate).toFixed(1) : '--'}%)`).join('\n') + '\n'
        : '  なし（本日分未生成）\n';
      msg += `\n追加: /focus add <銘柄> [メモ] / 解除: /focus del <銘柄>`;
      return sendTelegramTo(chatId, msg);
    } catch (e) { return sendTelegramTo(chatId, `[MAGI] /focus エラー: ${e.message}`); }
  }
  ...
  if (cmd === '/kill') {
    try {
      ...
      if ((args[0] || '').toLowerCase() === 'status') {
        ...
        if (s.unknown) {
          return sendTelegramTo(chatId, `[MAGI] \u26A0\uFE0F キルスイッチ状態不明\nsystem_control を読み取れません。magi-core はリスク増加注文のみブロック中です。`);
        }
        return sendTelegramTo(chatId, s.halted
          ? `[MAGI] \u{1F6D1} 緊急停止中\n理由: ${s.reason || 'なし'}\n発動: ${s.updated_by || '不明'}\n時刻: ${s.updated_at || '--'}\n解除: /resume`
          : `[MAGI] \u2705 通常稼働中（キルスイッチは解除状態）`);
      }
      const reason = args.join(' ') || null;
      await setKillSwitch(true, { reason, updatedBy: `telegram:${chatId}` });
      return sendTelegramTo(chatId,
        `[MAGI] \u{1F6D1} 緊急キルスイッチ発動\n` +
        `全注文（新規エントリー・決済とも）をL0ガードでブロックします\n` +
        (reason ? `理由: ${reason}\n` : '') +
        `解除: /resume`);
    } catch (e) { return sendTelegramTo(chatId, `[MAGI] /kill エラー: ${e.message}`); }
  }

  if (cmd === '/resume') {
    try {
      const s = await getKillSwitchStatus();
      if (s.halted === false && !s.unknown) {
        return sendTelegramTo(chatId, `[MAGI] キルスイッチは発動されていません（通常稼働中）`);
      }
      await setKillSwitch(false, { updatedBy: `telegram:${chatId}` });
      return sendTelegramTo(chatId, s.unknown
        ? `[MAGI] \u2705 キルスイッチを解除しました。取引を再開します。\n\u26A0\uFE0F 解除前の状態は確認できませんでした（system_control 読取不可）`
        : `[MAGI] \u2705 キルスイッチを解除しました。取引を再開します。`);
    } catch (e) { return sendTelegramTo(chatId, `[MAGI] /resume エラー: ${e.message}`); }
  }
