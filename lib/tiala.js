...
function buildActionScripts({ action, coordinate, text, key, keys, direction, amount }) {
  ...
  if (action === 'click' || action === 'left_click') {
    ...
    if (code == null && mainKey.length === 1 && modifiers.length > 0) {
      // Build a cliclick hotkey command for simple letter/modifier combinations.
      const modifierKeys = modifiers.map(m => m === 'command' ? 'cmd' : m === 'option' ? 'alt' : m === 'control' ? 'ctrl' : m).join(',');
      cliclickCmd = `kd:${modifierKeys} t:${mainKey} ku:${modifierKeys}`;
    }
...
async function performAction({ action, coordinate, text, key, keys, direction, amount }) {
  ...
  if (cliclickCmd) {
    ...
    const res = await callOpenClaw('exec', execArgs(script, 30));
  ...
  const res = await callOpenClaw('exec', execArgs(heredoc, 30));
