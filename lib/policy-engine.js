...
const COMMAND_POLICIES = {
  ...
  openclaw_agent: {
    risk: 'medium',
    requiresConfirmation: true,
    description: 'OpenClawエージェントによるTIALA操作（画面確認・コマンド実行等）',
    isDangerous: () => false,
    dangerReason: null
  },
  tiala_exec: {
  ...
  add_focus_symbol: {
    risk: 'low',
    requiresConfirmation: true,
    description: 'HERMES監視銘柄の手動追加',
    isBlocked: (cmd) => {
      const symbol = String(cmd.symbol || '').trim().toUpperCase();
      if (!/^[A-Z]{1,6}(?:[.\-][A-Z0-9]{1,3})?$/.test(symbol)) {
        return { blocked: true, reason: `無効なティッカーシンボル: ${cmd.symbol}` };
      }
      return { blocked: false };
    },
    isDangerous: () => false,
