...
const AKA1_TOOLS = [
  ...
  // --- TIALA operation tools ---
  {
    name: 'tiala_services',
    description:
      'TIALA (Mac mini M4) 上の全サービス（Ollama, OpenD, moomoo-bridge, OpenClaw等）の稼働状態を取得する。各サービスのポート疎通・プロセス有無を返す。',
    input_schema: { type: 'object', properties: {} }
  },
  {
    name: 'tiala_restart',
    description:
      'TIALA上の指定サービスを再起動する。対応: ollama, moomoo-bridge。手動再起動が必要なサービス（opend, openclaw）はエラーを返す。' +
  ...
  },
  {
    name: 'tiala_exec',
    description:
      'TIALA上で許可されたコマンドを実行する。allowlist方式: git, ollama, brew, ls, cat, ps, curl 等のプレフィックスのみ。' +
      'タイムアウト30秒、出力10KB制限。confirmed=true が必要。',
    input_schema: {
      type: 'object',
      properties: {
        command: {
  ...
  },
  {
    name: 'tiala_system',
    description:
      'TIALAのシステム情報を取得する。CPU, メモリ使用率, ディスク, 稼働時間, ロードアベレージを返す。',
    input_schema: { type: 'object', properties: {} }
  },
...
async function akaTool_tialaServices() {
  return callTiala('/tiala/services');
}
...
async function akaTool_tialaRestart({ service_name, confirmed }) {
  ...
  return callTiala(`/tiala/services/${encodeURIComponent(service_name)}/restart`, {
    method: 'POST'
  });
}
