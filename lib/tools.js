...
const AKA1_TOOLS = [
  ...
  },
  {
    ...
    description:
      'TIALAの画面をスクリーンショットで取得する。OpenClaw の computer ツールを優先し、利用不可な場合は macOS screencapture で取得する。',
    input_schema: { type: 'object', properties: {} }
  },
  {
    name: 'tiala_action',
    description:
      'TIALAの画面を操作する（OpenClaw computer ツール）。クリック、入力、キー操作、スクロール、アプリ起動など。' +
      '危険操作のため confirmed=true が必要。',
    input_schema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          description: 'アクション名（例: screenshot, click, double_click, right_click, type, type_chars, key, hotkey, scroll, move_mouse, drag, list_apps, list_windows, launch_app, get_app_state, get_screen_size, get_accessibility_tree, set_value, execute_javascript）'
        },
        coordinate: {
          type: 'array',
          items: { type: 'number' },
          minItems: 2,
          maxItems: 2,
          description: 'クリック/移動/ドラッグ先の [x, y] 座標（action=click 等で必要）'
        },
        text: {
          type: 'string',
          description: '入力する文字列（action=type/type_chars/set_value で使用）'
        },
        key: {
          type: 'string',
...
async function akaTool_openclawAgent({ instruction, model, session_key }) {
...
// Convert Anthropic tool schema to OpenAI function tools (Sakana AI uses OpenAI-compatible API)
