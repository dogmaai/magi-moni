  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        result: {
          type: 'string',
          description: '取引結果フィルター: WIN / LOSE / HOLD。省略時は全て'
        },
        limit: {
          type: 'integer',
...
async function akaTool_queryThoughts({ llm_provider, result, limit }) {
  ...
  if (result) {
    whereClauses.push('result = @result');
    params.result = result.toUpperCase();
  }
  ...
  const query = `
    SELECT
      llm_provider, symbol, action, reasoning, confidence, result, timestamp
