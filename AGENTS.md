...
> MAGI関連の全タスクは、作業開始・設計・実装後・レビュー・完了確認の各段階で、対象リポジトリがpinしている（未pinの場合はmainブランチの） dogmaai/magi-knowledge のOKFを参照する。報告には参照commit SHAとファイルパスを記載する。仕様とコードの矛盾は黙って解消せず、Junへ判断を求める。

### Canonical precedence

1. The OKF v0.2 bundle in `dogmaai/magi-knowledge` — at the revision
   pinned by this repository. **This repository does not currently pin a
   revision**; until a `vendor/magi-knowledge` submodule pin is added (as
   in `magi-core`), reference the bundle's `main` branch, record the
   exact commit SHA at task start, and use that same SHA consistently
   across all stages of the task to prevent mid-task specification drift.
2. Source code and running configuration (actual current behaviour).
3. Issues, PRs, conversations and prior handoff material.

`dogmaai/magi-stg` is archived and MUST NOT be referenced as a
specification. (Deployed services or datasets that happen to carry the
`magi-stg` name are operational monitoring targets, not spec sources.)
...
2. In that revision, review: `index.md`, `AGENTS.md`,
   ...
   `system/plm-units/`, `system/echidna-tables/`, `system/services/`, and
   the relevant `_lilith_safe/` boundaries (there is no local checkout in
   this repository — review everything on GitHub at the recorded SHA).
3. Check each document's frontmatter: `status`, `verified.by`,
   `stale_after`. Never use `deprecated` docs. Treat `draft` or
   AI-generated-but-unverified content as a hypothesis, not confirmed
   specification.
4. If canonical documents are unreachable or contradict each other,
   report to Jun instead of guessing.
...
Every start report, design proposal, PR description, review and
...
Changes to trade guards, order execution, trading modes, risk
configuration, the LILITH boundary, or Deep Research Section 5 strip
processing require review by someone other than the implementer.
Production deploys and GCP infrastructure changes are Jun's to approve
and execute — agents must not run them.
